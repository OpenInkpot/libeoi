...
static void key_down(void* param, Evas* e, Evas_Object* o, void* event_info)
{
    ...
    {
        if(++cur_sort == sorts_size) cur_sort = 0;
        edje_object_signal_emit(rr, sorts[cur_sort], "");
    }

    if(!strcmp(ev->keyname, "Return"))
    {
        if(++show_clock == 2) show_clock = 0;

        edje_object_part_text_set(rr, "clock", show_clock ? "57:95" : "--:--");
    }

    if(!strcmp(ev->keyname, "Up"))
    {
        if(++overlays_open == 4) overlays_open = 0;
    }
}
...
static void run()
{
   ...
   evas_object_event_callback_add(rr, EVAS_CALLBACK_KEY_DOWN, &key_down, NULL);
