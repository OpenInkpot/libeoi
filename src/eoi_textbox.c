...
static Eina_List* paginator(const Evas_Object* obj)
{
    ...
    Evas_Textblock_Cursor* c = evas_object_textblock_cursor_new(info->textblock);
    for(int i = 0;
        evas_object_textblock_line_number_geometry_get(info->textblock, i, NULL, &y, NULL, &h);
        i++)
    {
        evas_textblock_cursor_line_set(c, i);
        
        if(cur_y + info->parent_h < y + h && evas_textblock_cursor_node_text_length_get(c))
        {
            cur_y = y;
...
static void hide_last_line(const Evas_Object* obj)
{
    Evas_Coord x, y, w, h;
    ...
    for(int i = 0;
        evas_object_textblock_line_number_geometry_get(info->textblock, i, &x, &y, &w, &h);
        i++)
    {
...
static void textblock_resize_cb(void* data,
                                Evas_Object* obj,
                                Evas_Coord w,
                                Evas_Coord h)
{
    ...
    evas_object_geometry_get(info->parent, &parent_x, &parent_y, &parent_w, NULL);
                              ...
                              eoi_textbox_page_updated_t page_handler)
{
    ...
    Evas_Object* t = (Evas_Object*)edje_object_part_object_get(edje, "text");
...
void eoi_textbox_text_set(Evas_Object* obj, const char* text)
{
    edje_object_part_text_set(obj, "text", text);
}

bool eoi_textbox_page_next(Evas_Object* obj)
{
    eoi_textblock_info_t* info = evas_object_data_get(obj, "info");
...
bool eoi_textbox_page_prev(Evas_Object* obj)
{
    eoi_textblock_info_t* info = evas_object_data_get(obj, "info");
